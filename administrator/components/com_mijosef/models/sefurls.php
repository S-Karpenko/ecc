...
class MijosefModelSefUrls extends MijosefModel {
	...
	function getToolbarSelections() {
        ...
        $toolbar->action = JHTML::_('select.genericlist', $act, 'sefurls_action', 'class="inputbox" style="margin-top: 17px;" size="1" onchange="showInput();"');
        ...
        $toolbar->selection = JHTML::_('select.genericlist', $sel, 'sefurls_selection', 'class="inputbox" style="margin-top: 17px;" size="1"');
	...
	function delete($where) {
		...
		if ($this->MijosefConfig->delete_other_sef == '1') {
			$sef_urls = MijoDatabase::loadResultArray("SELECT url_sef FROM #__mijosef_{$this->_table}{$where}");
		}
	...
	function used($id) {
		...
		MijoDatabase::query("UPDATE #__mijosef_urls SET used = {$used} WHERE url_sef = '{$row->url_sef}' AND id != {$row->id} {$limit}");
	}
