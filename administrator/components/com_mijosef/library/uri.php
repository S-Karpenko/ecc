...
class MijosefURI {
	...
	function nonSefVars(&$uri, $non_sef_vars = null, $non_sef_part = "") {
		...
		if ($this->MijosefConfig->append_non_sef && isset($non_sef_vars)) {
			...
			$global_non_sef = $mainframe->get('mijosef.global.nonsefvars');
			if (!empty($global_non_sef)) {
				...
				$mainframe->set('mijosef.global.nonsefvars', $global_non_sef);
			}
		}			
	...
	function _finalizeSEF($uri, $sef_url, $real_url, $component, $lang_code) {
		...
		if ($real_url != "" && ($sef_url != "" || ($sef_url == "" && self::_isHomePage($uri)))) {
			...
			// Sitemap
			if ($this->MijosefConfig->sm_auto_mode == 1 && class_exists('MijosefSitemap')) {
				Mijosef::get('sitemap')->autoSitemap($component, $this->attributes->params, $sef_url, $real_url);
			}
		
			// Tags
			if ($this->MijosefConfig->tags_auto_mode == 1 && is_array($metadata) && !empty($metadata['keywords']) && class_exists('MijosefTags')) {
	...
	function parseURI($uri, $old_uri) {
        ...
        if (empty($route)) {
			...
			if (is_object($row) && Mijosef::get('utility')->getParam($row->params, 'published') == '1') {
				...
				if (is_object($meta)) {
					if (!empty($meta->title))  			$mainframe->set('mijosef.meta.title',		$meta->title);
					if (!empty($meta->description))   	$mainframe->set('mijosef.meta.desc',			$meta->description);
					if (!empty($meta->keywords))    	$mainframe->set('mijosef.meta.key',			$meta->keywords);
					if (!empty($meta->lang))   			$mainframe->set('mijosef.meta.lang',			$meta->lang);
					if (!empty($meta->robots)) 			$mainframe->set('mijosef.meta.robots',		$meta->robots);
					if (!empty($meta->google))			$mainframe->set('mijosef.meta.google',		$meta->google);
					if (!empty($meta->canonical)) 		$mainframe->set('mijosef.link.canonical',	$meta->canonical);
				}
			}
